...
const BlogCard = ({ i, admin = false, handler, blog, onPublish }) => {
    ...
    >
      ...
      <div className="relative w-full h-48 overflow-hidden">
        ...
        {admin && (
          ...
          >
            ...
            <span className="capitalize hidden xs:block md:hidden lg:block">
              {blog.status}
            </span>
          </div>
        )}
      ...
      <div className="p-2 flex flex-col">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">
          Published: {moment(blog.createdAt).format("DD - MM - YY")}
        </p>
        ...
        {/* Publish Button (Admin only, Draft Blogs) */}
        {admin && blog.status === "draft" && (
          <motion.button
            ...
            onClick={(e) => {
              e.stopPropagation();
              if (onPublish) onPublish(blog._id);
            }}
            className="flex items-center justify-center gap-2 w-full py-2 bg-purple-600 text-white font-semibold rounded-md shadow-md hover:bg-purple-700 cursor-pointer transition-all duration-300 delay-75"
          >
            <Upload size={18} />
            Publish Blog
          </motion.button>
